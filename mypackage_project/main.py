...
def format_group_report(group):
    ...
    report += f"👩‍🏫 Куратор: {group.curator if hasattr(group, 'curator') else 'Не указан'}\n"
    report += f"👨‍🎓 Количество студентов: {len(group.students)}\n"
    report += "\nСписок студентов:\n"
    report += "-" * 40 + "\n"
    
    for i, student in enumerate(group.students, 1):
        report += f"{i}. {student.first_name} {student.last_name}"
        if hasattr(student, 'age'):
            report += f" ({student.age} лет)"
...
def demo_package_usage():
    ...
    if hasattr(group, 'get_group_average'):
